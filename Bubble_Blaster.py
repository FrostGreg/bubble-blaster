...
def collision():
    ...
    for bub in range(len(bub_id) - 1, -1, -1):
        if distance(ship_id2, bub_id[bub]) < (SHIP_R + bub_r[bub]):  # checks for collision between bub and sub
...
while time() < end:
    if randint(1, BUB_CHANCE) == 1:
        # generates num(1-10) if = 1 then it generates # this reduces the amount of bubbles created
        create_bubble()
    move_bubbles()
    clean_up_bubs()
    score += collision()
